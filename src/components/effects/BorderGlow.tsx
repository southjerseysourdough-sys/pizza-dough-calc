 ...
 *   pnpm dlx shadcn@latest add @react-bits/BorderGlow-TS-TW \
 ...
 *     reduced motion and on touch devices where a pointer never hovers.
 *
 * The glow is driven purely by pointer position — `animated` stays false — so
 * nothing animates on its own and there is no render loop.
 */
...
interface BorderGlowProps {
  ...
  colors?: string[];
  fillOpacity?: number;
  /** Switches the effect off entirely, leaving a plain token surface. */
  disabled?: boolean;
}
...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  useEffect(() => {
    ...
    // The opening sweep is kicked off from a frame callback rather than
    // straight from the effect body. Setting state synchronously here would
    // cascade an extra render on mount, which React's hook rules reject.
  ...
  // Disabled keeps the surface completely static: no glow, no pointer work.
  const isVisible = !disabled && (isHovered || sweepActive);
