...
| ---------------------------- | -----------: | ----------: | ------------: | ---------: | --------------: | -----------: |
| Calculator `/`               |  1,476,437 B | 1,007,091 B |     432,750 B |  311,948 B |       369,962 B |    271,681 B |
| Baking Day `/bake`           |  1,657,526 B | 1,453,572 B |     496,116 B |  432,869 B |       426,252 B |    373,120 B |
| `/bake` minus `/`            |    181,089 B |   446,481 B |      63,366 B |  120,921 B |        56,290 B |    101,439 B |

The initial calculator is 469,346 bytes raw and 120,802 bytes gzip smaller: a
31.8% raw and 27.9% gzip reduction. It is below the 350 KB gzip launch target.
Baking Day is also 63,247 bytes gzip smaller in absolute terms. Its relative
...
First-load CSS moved from 89,321 bytes raw / 15,354 bytes gzip to 99,390 bytes
raw / 17,218 bytes gzip / 14,527 bytes Brotli. The 1,864-byte gzip increase
supplies launch dialogs, safe-area handling, loading/error states,
offline/install UI, and the readability preferences.
...
|   137,014 B |  41,628 B |  35,011 B | calculator/launch application code                                      |
...
- Atkinson Hyperlegible Next and IBM Plex Sans are self-hosted build assets;
  readability preferences add no runtime JavaScript dependency.
